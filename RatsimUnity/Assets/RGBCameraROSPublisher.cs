...
public class RGBCameraROSPublisher : MonoBehaviour
{
  ...
  void Start()
  {
    ...
    last_render_time = Time.time - (1f / this._camera.scanRate);
    /* Time.captureFramerate = (int)this._camera._scanRate; */
    Time.captureFramerate = 100;


    RosTimeController.onGlobalSensorsRequest += SenseAndPublish;
  }

  void SenseAndPublish()
  {
    ...
    // Send camera info TODO fix errors
  ...
  void FixedUpdate()
  {
    if (!RosTimeController.AsyncSensingAllowed())
    {
      return;
    }
