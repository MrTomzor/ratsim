...
class NavTask:
    def __init__(self, start_position, start_rotation_deg_clockwise, goal_bag_idx, goal_bag_step, max_distance_from_goal, max_steps):
        self.start_position = tuple(start_position)
        self.start_rotation_deg_clockwise = start_rotation_deg_clockwise
        self.goal_bag_idx = goal_bag_idx
        self.goal_bag_step = goal_bag_step
        self.max_distance_from_goal = max_distance_from_goal
        self.max_steps = max_steps

    def __repr__(self):
        return (f"NavTask(start_position={self.start_position}, "
                f"start_rotation_deg_clockwise={self.start_rotation_deg_clockwise}, "
                f"goal_bag_idx={self.goal_bag_idx}, "
                f"goal_bag_step={self.goal_bag_step}, "
                f"max_distance_from_goal={self.max_distance_from_goal}, "
...
class NavBenchmark:
    ...
    #     pass
    def __init__(self, bag_files=None, tasks=None, datasets_root=None):
    ...
    def from_folder(folder_path):
        ...
        for trial_data in tasks_data:
            task = NavTask(
                ...
                goal_bag_idx=trial_data["goal_bag_idx"],
                goal_bag_step=trial_data["goal_bag_step"],
                max_distance_from_goal=trial_data["max_distance_from_goal"],
                max_steps=trial_data["max_steps"]
            )
            tasks.append(task)

        return NavBenchmark(bag_files=bag_files, tasks=tasks, datasets_root=folder_path)
