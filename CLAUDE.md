...
- **`exploration_tracker.py`** — `ExplorationTracker`: 2D occupancy grid (nav_msgs convention: -1 unknown / 0 free / 100 occupied), axis-aligned, centered on world origin. `world_bounds/width` is the Unity X extent (= ROS y) and `world_bounds/height` the Unity Z extent (= ROS x), so the grid is `height` cells along ROS x and `width` cells along ROS y — before 2026-09 this was swapped and non-square worlds (e.g. 300x62 cell_grid) had ~4/5 of their area outside the grid. `update_from_lidar()` ray-casts via Bresenham, marks free/occupied cells and returns stats (`newly_known`, `rays_total`, `rays_out_of_bounds`, `rays_zero_len`, `agent_in_bounds`). `to_rgb_image()` renders the grid rotated/flipped to match Unity's top-down view (ROS +x / Unity +Z = top, ROS +y / Unity -X = left).
...
- **Wrong-well penalty** (wells task): with `well_schedule/wrong_well_detection=1` in the world (overlay preset `wrong_well_detection`), Unity publishes one `FloatArrayMessage` `[well_id, trial_type (0=random,1=home), reward_at_stake]` on `/well/wrong_activation` per dwell at a non-target well. Unity only *reports*; TaskTracker decides the cost from `foraging_settings`: `wrong_well_penalty_fraction` (default 0 = off; penalty = fraction × reward_at_stake × `reward_object_pickup_modifier`) and `wrong_well_penalty_trials` (`home` default | `random` | `both`). Every event is counted regardless — `get_wrong_well_activations()` → `{"random": n, "home": n}`. Reference task preset: `volumetric_exploration_2000_collision_penalty_wrong_well`.
...
- `ratsim_vis/trajectory_plot.py` — `plot_trajectories(ax, trajs, world_bounds=...)` draws recorded trajectories on a blank top-down slate in Unity frame (x right = ROS −y, z up = ROS +x), start/end/pickup markers; `colour_by_time=True` colours the line by step through `cmap` (pass the same `time_range=(t0, t1)` to every panel for a shared scale; returns the `ScalarMappable` for a colourbar). `background` + `view_proj` (Unity 4x4 view-projection) project the same points onto a rendered image via `project_to_pixels` — orthographic, isometric and perspective cameras share that path.
