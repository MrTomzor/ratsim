...
if __name__ == "__main__":
    ...
    refmap.construct_kdtree()
    print("KDTree constructed.")

    # Estimate map entropies
    print("Estimating place entropies...")
    refmap.estimate_places_entropy()
    print("Place entropies estimated.")

    # Find start and goal place indices
    # start_pos_idx = refmap.get_closest_place_idx(np.array([start_x, start_z, 0]))
    # goal_pos_idx = refmap.get_closest_place_idx(np.array([goal_x, goal_z, 0]))
    ...
    while True:
        ...
        for j in modes:
            ...
            print("Start place idx: " + str(start_pos_idx) + ", goal place idx: " + str(goal_pos_idx))
            import time
            start_time = time.time()
            path_idx_list = None
            endcost = None
            closed_set = None
            print("Searching path with mode " + j)
            ...
            if path_idx_list is None:
                print("No path found!")
                break
            respaths.append(path_idx_list)
            print("Path found with " + str(len(path_idx_list)) + " places, endcost: " + str(endcost))
    ...
    # select random start and goal places
